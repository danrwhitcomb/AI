...
public class CarDriver extends Application {
	...
	public void start(Stage primaryStage) {
		...
		CarState state2 = new CarState(30, 200, 0);
	    // Set CarState;
		car.set(state1);
		
		RRT rrt = new RRT(w, window_height, window_width);
		
		LinkedList<CarState> path = rrt.runRRT(state1, state2, car);
		
		for(CarState st : path){
			plotCarRobot(g, car, st);			
		}
